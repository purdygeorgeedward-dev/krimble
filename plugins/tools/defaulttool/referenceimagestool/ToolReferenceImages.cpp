...
void ToolReferenceImages::pasteReferenceImage()
{
    ...
    if (reference) {
        ...
        if (canvas()->canvasWidget()) {
            QMessageBox::critical(canvas()->canvasWidget(), i18nc("@title:window", "Krimble"), i18n("Could not load reference image from clipboard"));
        }
    }
}
...
void ToolReferenceImages::loadReferenceImages()
{
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        QMessageBox::critical(qApp->activeWindow(), i18nc("@title:window", "Krimble"), i18n("Could not open '%1'.", filename));
        return;
    }
    ...
    if (collection.load(&file)) {
        ...
        QMessageBox::critical(qApp->activeWindow(), i18nc("@title:window", "Krimble"), i18n("Could not load reference images from '%1'.", filename));
    }
    file.close();
}
...
void ToolReferenceImages::saveReferenceImages()
{
    ...
    if (!file.open(QIODevice::WriteOnly)) {
        QMessageBox::critical(qApp->activeWindow(), i18nc("@title:window", "Krimble"), i18n("Could not open '%1' for saving.", filename));
        return;
    }
    ...
    if (!ok) {
        QMessageBox::critical(qApp->activeWindow(), i18nc("@title:window", "Krimble"), i18n("Failed to save reference images."));
    }
}
