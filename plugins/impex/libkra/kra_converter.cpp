...
KisImportExportErrorCode KraConverter::buildImage(QIODevice *io)
{
    ...
    if (m_store->bad()) {
        m_doc->setErrorMessage(i18n("Not a valid Krimble file"));
...
QDomDocument KraConverter::createDomDocument()
{
    ...
    root.setAttribute("editor", "Krimble");
...
KisImportExportErrorCode KraConverter::loadXML(const QDomDocument &doc, KoStore *store)
{
    ...
    if (syntaxVersion > 2) {
        errUI << "The file is too new for this version of Krimble:" << syntaxVersion;
        m_doc->setErrorMessage(i18n("The file is too new for this version of Krimble (%1).", syntaxVersion));
