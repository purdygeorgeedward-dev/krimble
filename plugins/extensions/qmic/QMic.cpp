...
void QMic::slotQMic(bool again)
{
    ...
    for (const auto &loader : offers) {
        ...
        if (!factory) {
            warnPlugins << "(GMic) This is not a Krimble plugin: " << loader.fileName() << loader.errorString();
    ...
    if (!plugin) {
        QMessageBox::warning(qApp->activeWindow(), i18nc("@title:window", "Krimble"), i18n("Krimble cannot launch the gmic-qt plugin. No bundled library found."));
        return;
    }
