...
PaletteDockerDock::PaletteDockerDock( )
    ...
    , m_actEditPalette(new QAction(KisIconUtils::loadIcon("palette-edit"), i18n("Edit current swatches (palette)")))
    , m_actSavePalette(new QAction(KisIconUtils::loadIcon("document-save-16"), i18n("Save current swatches (palette)")))
    , m_colorSelfUpdate(false)
{
    ...
    m_ui->bnColorSets->setToolTip(i18n("Load Swatches (Palette)"));
...
void PaletteDockerDock::slotExportPalette(KoColorSetSP palette)
{
    ...
    dialog.setCaption(i18n("Export Swatches (Palette)"));
    ...
    if (palette->saveToDevice(&file)) {
        m_view->showFloatingMessage(
            i18nc("Floating message about exporting successful", "Swatches (Palette) exported successfully"), QIcon(),
...
void PaletteDockerDock::slotUpdateLblPaletteName()
{
    if (m_currentColorSet) {
        ...
        if (isGlobal) {
            m_actSavePalette.data()->setToolTip(i18nc("@tooltip", "Save swatches (palette) explicitly, will also happen automatically on exiting Krimble."));
        }
        else {
            m_actSavePalette.data()->setToolTip(i18nc("@tooltip", "Saving for document swatches (palettes) is done by saving the document."));
        }
