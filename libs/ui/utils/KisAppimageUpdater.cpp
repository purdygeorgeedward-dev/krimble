...
void KisAppimageUpdater::slotUpdateFinished(int result, QProcess::ExitStatus exitStatus)
{
    ...
    if (exitStatus == QProcess::CrashExit) {
        ...
        case 0:
            ...
            statusDetails = i18n("New AppImage was downloaded to %1. To complete the update, close Krimble and run the new AppImage.", finfoAppImagePath.path());
            break;
        default:
            // some errors have exit code of 255 (modified by system, when AppImageUpdate returns -1)
            updateStatus = UpdaterStatus::StatusID::UPDATE_ERROR;
...
bool KisAppimageUpdater::findUpdaterBinary()
{
    ...
    if (finfo.isExecutable()) {
        ...
        KisUsageLogger::log(
                    QString("KisAppimageUpdater: AppImageUpdate (%1) was not found within the Krimble appimage, or is not executable")
                    .arg(m_updaterBinary)
                    );
        return false;
    }
