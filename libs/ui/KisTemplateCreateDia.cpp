    ...
    , d(new KisTemplateCreateDiaPrivate(templatesResourcePath, filePath, thumbnail))
{
    ...
    d->m_defaultTemplate->setToolTip(i18n("Use the new template every time Krimble starts"));
...
void KisTemplateCreateDia::slotSelect() {
    ...
    QImage image(fn);
    if (image.isNull()) {
        QMessageBox::warning(this, i18nc("@title:window", "Krimble"), i18n("%1 is not a valid image file!", fn));
    }
    d->m_customFile = fn;
