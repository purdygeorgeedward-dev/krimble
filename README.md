...
11. Limit move default to selected layer or floating selection until deliberately changed by user. (DONE — already satisfied by existing default, no code change needed. `KisToolMove::moveToolMode()` defaults to `MoveSelectedLayer` on any fresh install (persisted config key `moveToolMode` defaults to `0`, that enum value), confirmed in `KisNodeSelectionRecipe::selectNodesToProcess()` to mean strictly the already-selected node(s) — no cursor-based auto-pick of whatever layer is under the mouse. Switching to auto-pick requires deliberately toggling "Move First Layer" in tool options. Krita has no distinct "floating selection" object — paste creates a layer directly, and Move already respects whatever selection mask is active on the target layer as part of normal stroke mechanics, so that half of the requirement doesn't need separate handling.)
